...
class DQNCritic(BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        if self.double_q:
            ...
            if torch.rand(1) < 0.5:
                q_tp1 = torch.gather(qa_tp1_values, 1, torch.argmax(q_next, dim=1).unsqueeze(1)).squeeze(1)
            else:
                q_tp1 = torch.gather(q_next, 1, torch.argmax(qa_tp1_values, dim=1).unsqueeze(1)).squeeze(1)
